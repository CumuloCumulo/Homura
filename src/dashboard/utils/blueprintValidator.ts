...
/**
 * Validate variable references in Blueprint
 */
export function validateVariableReferences(blueprint: Blueprint): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  // Collect all defined variables from skill parameters
  ...
  // Check selector_logic for variable references
  for (const tool of blueprint.skills) {
    const referencedVars = extractVariables(tool.selector_logic);

    for (const varName of referencedVars) {
      if (!definedVars.has(varName)) {
        errors.push(
          `Undefined variable "${varName}" referenced in tool "${tool.tool_id}"`,
        );
      }
    }
  }
...
/**
 * Extract variable references from selector_logic
 */
function extractVariables(selectorLogic: unknown): string[] {
