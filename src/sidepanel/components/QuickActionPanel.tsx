...
export function QuickActionPanel({
  ...
  const defaultAnchorValue =
    unifiedSelector?.structureData?.anchor?.value ||
  ...
  const buildPayload = () => {
    ...
    return {
      scopeSelector:
        selectorDraft?.scope?.selector || analysis.containerSelector,
      anchorSelector:
        ...
        {extractedText !== null && (
          <div className="p-2 rounded bg-black/30 border border-zinc-800">
            ...
            <p className="text-[10px] text-emerald-400 font-mono break-all">
              {extractedText || (
                <span className="text-zinc-600 italic">(空)</span>
              )}
            </p>
          </div>
        )}
