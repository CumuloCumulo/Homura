...
import { executeTool, clearAllHighlights } from "./engine";
import {
  executeClick,
  executeInput,
  executeExtractText,
...
/** Transparent overlay that captures all events (like Chrome DevTools) */
...
/**
 * Start inspect mode with transparent overlay (Chrome DevTools style)
 *
 * This approach uses a transparent overlay to capture all mouse events,
 * preventing the page from receiving them. This allows us to:
 * 1. Highlight elements that would normally intercept events (like jqxWidget dropdowns)
 * 2. Click elements without triggering their event handlers
 *
 * z-index hierarchy:
 * - Overlay: 2147483647 (captures events)
 * - Highlight: 2147483646 (shows hover state)
 * - Page elements: normal z-index
 */
function handleStartInspect(): { success: boolean } {
  ...
  inspectOverlay.id = "homura-inspect-overlay";
  ...
  Object.assign(inspectOverlay.style, {
    ...
    cursor: "crosshair",
    zIndex: "2147483648", // Above max z-index (jqxWidget uses 2147483647)
  ...
  // Listen on overlay instead of document
  // Use capture phase (true) to intercept events before they reach page elements
  inspectOverlay.addEventListener("mousemove", onOverlayMouseMove, true);
  inspectOverlay.addEventListener("click", onOverlayClick, true);
  inspectOverlay.addEventListener("mouseleave", onOverlayMouseLeave, true);
  // Also block mousedown/mouseup to prevent dropdowns from closing
...
/**
 * Stop inspect mode and remove overlay
 */
function handleStopInspect(): { success: boolean } {
  ...
  if (inspectOverlay) {
    inspectOverlay.removeEventListener("mousemove", onOverlayMouseMove, true);
    inspectOverlay.removeEventListener("click", onOverlayClick, true);
    inspectOverlay.removeEventListener("mouseleave", onOverlayMouseLeave, true);
    inspectOverlay.removeEventListener("mousedown", onOverlayMouseDown, true);
    inspectOverlay.removeEventListener("mouseup", onOverlayMouseUp, true);
 ...
 * Uses elementFromPoint to find the actual element under cursor
 * Uses pointer-events: none to prevent triggering hover events on page elements
 */
function onOverlayMouseMove(e: MouseEvent): void {
  if (!isInspectMode || !inspectOverlay) return;

  const x = e.clientX;
  const y = e.clientY;

  // Temporarily disable pointer events to let mouse "pass through" without triggering events
  // This is better than display: none because it doesn't cause layout reflow
  const originalPointerEvents = inspectOverlay.style.pointerEvents;
  inspectOverlay.style.pointerEvents = "none";

  // Get element at point - the element won't receive any events because we're just querying
  const element = document.elementFromPoint(x, y) as HTMLElement;

  // Immediately restore pointer events to capture next mouse move
  inspectOverlay.style.pointerEvents = originalPointerEvents;
  ...
  // Update hovered element and show highlight
 ...
 * Selects element without triggering page events
 * Uses pointer-events: none to prevent click events from propagating to page elements
 */
function onOverlayClick(e: MouseEvent): void {
  if (!isInspectMode || !inspectOverlay) return;

  // Stop the click event from propagating to the page
  // But don't preventDefault() as it may interfere with browser behavior
  e.stopPropagation();

  const x = e.clientX;
  const y = e.clientY;

  // Temporarily disable pointer events to query element without triggering events
  const originalPointerEvents = inspectOverlay.style.pointerEvents;
  inspectOverlay.style.pointerEvents = "none";

  // Get element at point - no events are triggered on the element
  const element = document.elementFromPoint(x, y) as HTMLElement;

  // Immediately restore pointer events
  inspectOverlay.style.pointerEvents = originalPointerEvents;
  ...
  // Clear previous validation highlight
  removeValidationHighlight();

  // Show persistent selected highlight
  ...
  console.log("[Homura] Element selected via overlay:", {
    tag: element.tagName,
...
/**
 * Handle mouse leaving overlay (e.g., going to browser chrome)
 */
function onOverlayMouseLeave(): void {
 ...
 * Blocks the event from propagating to page elements (e.g., preventing dropdowns from closing)
 */
function onOverlayMouseDown(e: MouseEvent): void {
  if (!isInspectMode) return;
  // Stop propagation but don't prevent default to avoid interfering with browser behavior
  e.stopPropagation();
}

/**
 * Handle mouse up on overlay
 * Blocks the event from propagating to page elements
 */
...
async function handleExecuteWithLogic(
  ...
  try {
    ...
    if (scopeSelector) {
      ...
      // CRITICAL: Handle self-targeting (empty string or :scope means scope IS the target)
      if (
        !targetSelector ||
        targetSelector === "" ||
      ...
      usedSelector = `${scopeSelector} >> ${anchorSelector || "(无锚点)"} = "${anchorValue}" >> ${targetSelector === ":scope" ? "(self)" : targetSelector || "(self)"}`;
