...
● 去画布化 (No-Code Graph)：放弃复杂的节点连线图。用户不定义”路径”，只定义”能力（Skills）”和”目标（规则）”。
● 大脑与肢体解耦 (Brain-Limb Decoupling)：
  ○ AI (大脑)：负责非确定性的决策（判断条件、选择策略、处理异常）。
  ○ 插件引擎 (肢体)：负责确定性的执行（高精度点击、数据提取、循环遍历）。
● 声明式执行 (Declarative Execution)：
  ○ 用户定义”做什么”（What）- 通过 Rule Book 描述业务规则
  ○ AI 决定”怎么做”（How）- 根据页面状态动态选择 Skills
  ○ 引擎精确执行 - 毫秒级 DOM 操作
● 稳定性优先：在浏览器插件环境下，不依赖坐标，而是依赖”语义锚点”和”容器作用域”来定位元素。

**可选优化：混合动力循环 (Hybrid Loop - 可选)**
对于批量处理场景，可以选择性地使用确定性外循环来提升性能：
  ○ 外层循环（如遍历列表）由代码控制，保证绝对稳定和速度（可选）
  ○ 内层逻辑（如单条审核）由 AI 接管，保证灵活适应变化（核心）
  ○ 注意：这并非必需架构，纯 AI 迭代同样可以完成所有任务
...
**示例**：
```
Rule Book: "审批所有待处理的教室分配请求"

AI Agent 执行：
迭代 1: 获取列表 → 观察有 10 条待处理
迭代 2: 选择第一条 → 进入详情页
迭代 3: 检查校区 → 决定搜索仙林教室
迭代 4: 选择教室 → 容量符合要求
迭代 5: 确认提交 → 返回列表
...
**缺点**：
- 需要用户/开发者预先知道"有 100 条待处理"
- 不如纯 AI 灵活
- 增加了系统复杂度

**注意**：这是**可选优化**，并非必需架构。大多数场景下，纯 AI 迭代（模式 1）已经足够。
...
| 场景 | 推荐模式 | 原因 |
|------|----------|------|
| 不确定数量的任务 | 模式 1：纯 AI | AI 自主判断何时结束 |
| 复杂条件分支 | 模式 1：纯 AI | AI 动态决策，无需预定义 |
| 明确的大批量处理 | 模式 2：混合 | 性能优化 |
| 学习/调试阶段 | 模式 1：纯 AI | 更简单，更易理解 |

**核心原则**：优先使用模式 1（纯 AI），只在性能成为瓶颈时考虑模式 2（混合）。
